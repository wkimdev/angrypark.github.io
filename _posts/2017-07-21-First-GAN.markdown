...
<h6>saturate?</h6> 활성화 함수로 sigmoid를 가지는 아주 깊은 MLP의 경우를 생각해보자. 학습을 거치며 값들은 0이나 1에 가까워지게 되고 vanishing gradient문제가 발생하게 된다. 따라서 더이상 학습이 진행되지 않거나, 아주 느리게 진행된다. 이러한 경우를 neuron이 saturate되었다고 한다.
